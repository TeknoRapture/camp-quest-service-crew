...
let actionQueued = false, dialogueOpen = true, toastTimer = 0, hazardTick = 0;
...
function interact(){
  ...
  if(dist(player,{x:1210,y:430,w:40,h:40})<100){showDialogue('Beware of Cliff!', 'Someone scratched a strange symbol beneath the warning. Beyond it, misplaced crates block the bridge to the Back 40.');return;}
...
function draw(){
  ...
  hazards.forEach(h=>{ctx.globalAlpha=.7;ctx.fillStyle=h.kind==='mud'?'#69553c':h.kind==='wet'?'#68a9c7':'#738044';ctx.beginPath();ctx.ellipse(h.x+h.w/2,h.y+h.h/2,h.w/2,h.h/2,0,0,7);ctx.fill();ctx.globalAlpha=1;text(h.kind==='mosquitoes'?'•  •  •':h.label,h.x+h.w/2,h.y+h.h/2,10,'#e9dcaf')});
  ...
  ctx.fillStyle='#fff';ctx.fillRect(1175,300,74,92);ctx.strokeStyle='#111';ctx.lineWidth=4;ctx.strokeRect(1175,300,74,92);text('BEWARE',1212,321,10,'#e33');text('of',1212,338,9,'#e33');text('CLIFF!',1212,357,11,'#e33');ctx.strokeStyle='#e33';ctx.strokeRect(1192,365,40,20);ctx.beginPath();ctx.arc(1212,375,8,0,7);ctx.stroke();
  supplies.filter(s=>!s.done).forEach(s=>{ctx.fillStyle=s.id==='crate'?'#c58a45':'#ffe16b';ctx.fillRect(s.x,s.y,s.w,s.h);text(s.label,s.x+s.w/2,s.y-6,10,'#fff3ae')});
  ...
  ctx.fillStyle='#edb13d';ctx.beginPath();ctx.arc(player.x+12,player.y+9,10,0,7);ctx.fill();ctx.fillStyle='#d95637';ctx.fillRect(player.x,player.y+18,24,12);ctx.fillStyle='#254c76';ctx.fillRect(player.x+3,player.y+29,7,9);ctx.fillRect(player.x+14,player.y+29,7,9);text('YOU',player.x+12,player.y-6,10,'#fff');ctx.restore();
}
...
addEventListener('keydown',e=>{if(keyMap[e.key]){keys.add(keyMap[e.key]);e.preventDefault();}if([' ','e','E'].includes(e.key)){actionQueued=true;e.preventDefault();}if(e.key==='Escape')closeDialogue();});addEventListener('keyup',e=>{if(keyMap[e.key])keys.delete(keyMap[e.key]);});
