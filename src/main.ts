...
function draw(){
  ...
  hazards.forEach(h=>{if(h.assetId&&drawSprite(ctx,assets,h.assetId,h)){text(h.label,h.x+h.w/2,h.y-5,10,'#e9dcaf');return;}ctx.globalAlpha=.7;ctx.fillStyle=h.kind==='mud'?'#69553c':h.kind==='wet'||h.kind==='water'?'#68a9c7':'#738044';ctx.beginPath();ctx.ellipse(h.x+h.w/2,h.y+h.h/2,h.w/2,h.h/2,0,0,7);ctx.fill();ctx.globalAlpha=1;text(h.kind==='mosquitoes'?'•  •  •':h.label,h.x+h.w/2,h.y+h.h/2,10,'#e9dcaf')});
  ...
  supplies.filter(s=>!s.done).forEach(s=>{if(s.assetId&&drawSprite(ctx,assets,s.assetId,s)){text(s.label,s.x+s.w/2,s.y-6,10,'#fff3ae');return;}ctx.fillStyle=s.id==='crate'?'#c58a45':'#ffe16b';ctx.fillRect(s.x,s.y,s.w,s.h);text(s.label,s.x+s.w/2,s.y-6,10,'#fff3ae')});
